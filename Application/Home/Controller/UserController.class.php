...
class UserController extends HomeController{
    ...
    public function checkdata() {
        $one = 86400;
        $where = array(
            'check_in' => array('gt', mktime(0,0,0,date('m'),date('d'),date('y')) - 7 * $one)
        );
        $list = M('log')->where($where)->field('log_id, sa_uid, is_other, start_timestamp, check_in')->select();

        echo '<pre>';
        $sa = getSalesAgentList('');
        $other = 0;
        $wrong = 0;
        foreach ($list as $value) {
            ...
            if($sa_uid && $sa[$sa_uid]) {
                if($value['is_other'] == 1) {
                    var_dump(date('Y-m-d H:i:s', $value['start_timestamp']));
                    var_dump($value);
                    $wrong += 1;
                }
            }else {
        ...
        var_dump('wrong: '.$wrong);
        var_dump('other: '.$other);


        $where['is_other'] = 1;
        $count = M('log')->where($where)->count();
        var_dump('is other count: '.$count);
    }
