...
class SystemConfigController extends AdminController{
    ...
    public function add(){
        if(IS_POST){
            ...
            $builder->setMetaTitle('新增配置')  //设置页面标题
                    ...
                    ->addFormItem('group_', 'select', '配置分组', '配置所属的分组', C('CONFIG_GROUP_LIST'))
    ...
    public function group($group = 1){
        ...
        foreach($data_list as &$data){
            ...
            $data['value'] = stream_get_contents($data['value']);
    ...
    public function groupSave($config){
        if($config && is_array($config)){
            ...
            foreach ($config as $name => $value){
                ...
                $config_object->where($map)->save(['value' => $value]);
            }
        }
        S('DB_CONFIG_DATA',null);
        $this->success('保存成功！');
