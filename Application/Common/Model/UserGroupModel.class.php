...
class UserGroupModel extends Model{
    ...
    public function checkMenuAuth(){
        ...
        $user_group = (int)D('User')->getFieldById(session('user_auth.uid'), 'group_'); //获得当前登录用户信息
        if($user_group !== 1){
            $group_info = $this->find($user_group);
            $group_info['menu_auth'] = stream_get_contents($group_info['menu_auth']);
            session('menu_auth', $group_info['menu_auth']);
            // var_dump($group_info['menu_auth']);die;
